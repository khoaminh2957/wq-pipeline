...
def mint_gap(now=None):
    ...
    nearly-spent budget stretches to the reset rather than stopping dead.
    """
    now = now if now is not None else time.time()
...
def one_window():
    ...
    if r.status_code != 401 or r.headers.get("WWW-Authenticate") != "persona":
        ...
        if throttled:
            # A biometrics throttle is not a rate hiccup; it is the platform saying the persona
            # flow itself has been used too much. Retry-After is absent on it, so back off hard.
    ...
    try:
        ...
        print(f"notify: queued {_mid}", flush=True)
